...
export default {
  ...
  setup(props, context) {
    ...
    // })
    const tiny = reactive({
      apiKey: 'kvekuf5v3qt7dbbn05tm05g03j68rfcsi2qenlujp52psjse', //https://github.com/tinymce/tinymce-vue/blob/main/src/demo/views/Iframe.vue
      init: {
        menubar: false,
        language: 'zh_CN', //语言类型
        ...
        plugins:
          'codesample link image table lists fullscreen  code media',
        toolbar: [
          'bold italic underline h1 h2 blockquote codesample numlist bullist link image media | removeformat fullscreen wordcount',
        ], //工具栏配置，设为false则隐藏
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let {code,data,msg} = await uploadFile(form);
            console.log('/api/'+data.file.url)
            if(code === 0){
              if(data.file.url.slice(0, 4) !== 'http'){
                resolve('/api/'+data.file.url)
              }else{
        ...
        file_picker_types: 'image media', //file image media分别对应三个类型文件的上传：link插件，image和axupimgs插件，media插件。想屏蔽某个插件的上传就去掉对应的参数
        // 文件上传处理函数
