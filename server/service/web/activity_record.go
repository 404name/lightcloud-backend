...
// GetActivityRecordInfoDetailList 分页获取ActivityRecord详细记录
// Author [piexlmax](https://github.com/piexlmax)
func (activityRecordService *ActivityRecordService) GetActivityRecordInfoDetailList(info webReq.ActivityRecordSearch) (list []web.ActivityRecord, total int64, err error) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	// 创建db
	db := global.GVA_DB.Model(&web.ActivityRecord{})
	...
	err = db.Limit(limit).Offset(offset).Preload("Activity").Find(&activityRecords).Error
	return activityRecords, total, err
}
